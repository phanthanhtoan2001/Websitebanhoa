      ...
      <div className='flex flex-col gap-2 w-full'>
        ...
        <div className="flex justify-between mt-3 items-center">
          <div className='flex gap-3 items-center'>
            <button className='bg-gray-400 hover:bg-slate-500 mt-4 py-1 rounded-full p-2 w-8 h-8' onClick={() => dispacth(increaseQuanity(id))}><BiPlusMedical /></button>
            <p className='font-semibold p-2 text-xl mt-3'>{quanity}</p>
            <button className='bg-gray-400 hover:bg-slate-500 mt-4 py-1 rounded-full p-2 w-8 h-8' onClick={() => dispacth(decreaseQuanity(id))}><TiMinus /></button>
          </div>
