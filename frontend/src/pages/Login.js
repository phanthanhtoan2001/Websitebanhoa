  ...
  const handleSubmit = async (e) => { //Hàm xử lý khi người dùng submit form
    ...
    if(email && password) { //Nếu đầy đủ cả email và password thì gửi request lên server kiểm tra đăng nhập
      try {
        ...
        console.log(dataRes)
        toast(dataRes.message) //Hiển thị message từ server (thành công hay thất bại)
        if(dataRes.alert){ //Nếu đăng nhập thành công, gửi action thông qua dispatch(), chuyển về trang chủ sau 1s
    ...
    } else { //Nếu không đầy đủ email hoặc password, hiển thị thông báo yêu cầu nhập đủ
